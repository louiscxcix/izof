...
---
### [요약 보고서]
- **[종합 평가]**: 현재 선수의 상태를 한두 문장으로 명확하게 요약하라. (예: "전반적인 기술 숙련도는 안정적이지만, 시합 상황에서의 심리적 압박을 관리하고 순발력을 강화하는 훈련이 시급합니다.")
- **[주요 강점]**: 현재 점수가 필요 점수와 가장 근접하거나 초과하여 가장 잘하고 있는 **핵심 강점 2가지**를 키워드로 제시하라.
- **[주요 보완점]**: '요구 점수'와 '현재 점수'의 차이가 가장 커서 개선이 가장 시급한 **핵심 보완점 2가지**를 키워드로 제시하라.
---
### [상세 보고서]
1.  **[종합 분석 및 진단]**:
    - **맥락 파악:** 먼저, 입력된 데이터 항목들을 보고 이것이 어떤 스포츠(예: 골프, 수영, 야구 등)에 관련된 검사인지, 혹은 일반적인 심리 검사인지 추론하여 언급하라.
    - **총평:** 데이터 전반을 기반으로 선수의 현재 상태를 종합적으로 평가하라. 강점과 약점이 경기력에 어떻게 상호작용하고 있는지 심층적으로 설명하라. (예: "기술적인 자신감(강점)은 충분하지만, 이로 인해 오히려 큰 경기에서의 긴장(약점)이 더 크게 느껴질 수 있습니다.")
...
3.  **[솔루션 및 훈련 제안]**:
    - **핵심 문제 정의:** 위 분석을 바탕으로, 선수가 겪고 있을 가장 핵심적인 문제 상황을 구체적으로 정의하라. (예: "중요한 숏 퍼팅 상황에서 부정적인 생각이 개입되며, 이는 신체적 긴장으로 이어져 기술적 실수를 유발하고 있습니다.")
    - **맞춤형 훈련법:** 정의된 핵심 문제를 해결하기 위한 구체적인 훈련법 2가지를 심리적, 기술적, 체력적 관점을 통합하여 제안하라.
"""
    return prompt
